...
const LOT = ({ imageUrl, text, subText, reverse, center, textFont, subTextFont, textColor }: LOTProps) => {
  return (
    <motion.div
      ...
      className={`${reverse && textFont !== 'playfair' ? 'sh:flex-col-reverse flex-col' : 'flex-col'}
        ${reverse && textFont === 'playfair' ? 'md:flex-col-reverse flex-col' : 'flex-col'}
        flex sh:items-start items-center sm:gap-6 text-[#1F262B]
        ${center ? 'items-center' : ''}`
      }
    >
      {imageUrl && (
        <motion.img
          ...
          src={imageUrl}
          alt="LOT"
          className={`w-[600px] sh:h-auto h-[330px] object-cover rounded-md
      ...
      <motion.div
        ...
        className="lg:space-y-5 sm:space-y-3 space-y-1 sh:text-start text-center"
      >
        {text && (
          <motion.h2
            variants={{
        ...
        {subText && (
          <motion.p
            ...
            className={`${textColor ? 'text-white' : 'text-[#1F262B]'} ${subTextFont === 'manrope' ? 'font-manrope font-medium sh:max-w-[290px]' : subTextFont === 'norm' ? 'sm:text-[24px] text-[18px] font-normal' : subTextFont === 'norms' ? 'sm:text-[16px] text-sm font-medium' : 'sm:text-[21px] mt-2 text-[18px] text-center font-medium'}`}
          >
            {subText}
          </motion.p>
        )}

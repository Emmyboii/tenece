...
const OurStoryGrid = ({ image, title, text }: OurStoryGridProps) => {
    ...
    return (
        <div className="flex rx:flex-row flex-col rx:text-start text-center items-center justify-between gap-5 mt-20 text-[#1F262B]">
            <img src={image} alt="" className={`${location === '/blog' && '3xl:w-[548px] w-full rounded-[4px]'} rx:w-[548px] w-full rounded-[4px]`} />
            <div className={`sm:space-y-7 space-y-4 rx:max-w-[678px] ${location === '/blog' && '3xl:max-w-[527px]'}`}>
                <p className={`${location === '/blog' ? 'mk:text-[40px] text-[24px] font-medium' : 'font-playfair mk:text-[64px] sm:text-[45px] text-[32px] font-semibold'}`}>{title}</p>
